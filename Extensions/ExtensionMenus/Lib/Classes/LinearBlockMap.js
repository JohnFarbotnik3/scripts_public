...
class LinearStorageBuffer {
	constructor(args, fget, fset, fdel, fsize) {
		// storage functions.
		this.args = args;
		...
		// queues.
		this.qset = new Map();// Map<id, block>
		this.qdel = new Set();// Set<id>
	}
	get   (k  ) { return this.fget(this.args, k); }
	set   (k,v) { this.qset.set(k,v);  this.qdel.delete(k); }
	delete(k  ) { this.qset.delete(k); this.qdel.add(k); }
	async commit() {
		// check if queues are empty.
		if(this.qdel.size === 0 & this.qset.size === 0) {
			return Promise.resolve(true);
		}
		//console.log("commit->start");
		const proms = [];
		for(const  k    of this.qdel.keys   ()) proms.push(this.fdel(this.args, k   ));
		for(const [k,v] of this.qset.entries()) proms.push(this.fset(this.args, k, v));
...
class LinearBlockMap {
	...
	async flush() {
		...
		await this.storage.commit();// commit what we already have.
		await this.previousOperation;
		await this.storage.commit();// commit once chain of operations is completed.
		return true;
	}
	// save block.
	saveBlock(block) {
	...
	async load() {
		...
		this.blocks.clear();
		// check if map exists.
		const exists = await this.storage.get("exists");
