...
async function TEST_1() {
	console.log("TEST_1");
	const linbuf = new LinearStorageBuffer("prefix", _fget, _fset, _fdel, _fsize);
...
async function TEST_2() {
	...
	const arr_n = [0, 1, 10, 100, 500];
	const arr_m = [10, 100, 10000, 100000];
	const arr_w = [[0.5, 0.7], [0.7, 0.1], [0.3, 0.9], [0.01, 0.01], [0.99], [Math.random(), Math.random()]];
	for(const [lo,hi] of arr_sz)
	for(const n of arr_n)
	for(const m of arr_m)
	for(const [w1,w2] of arr_w) test_params.push([lo, hi, n, m, w1, w2]);
	// test
	try {
	for(const params of test_params) {
		...
		const [MINSZ, MAXSZ, N, M, WSET1, WSET2] = params;
		const linbuf = new LinearStorageBuffer("prefix", _fget, _fset, _fdel, _fsize);
		...
		for(let x=0;x<N;x++) {
			...
			if(OP < WSET1)	tset(linmap, Math.floor(Math.random()*M), x);
			else			tdel(linmap, Math.floor(Math.random()*M));
		}
		for(let x=0;x<N;x++) {
			const OP = Math.random();
			if(OP < WSET2)	tset(linmap, Math.floor(Math.random()*M), x);
			else			tdel(linmap, Math.floor(Math.random()*M));
		}
		for(let x=0;x<N;x++) {
			const OP = Math.random();
			if(OP < WSET1)	tset(linmap, Math.floor(Math.random()*M).toString(), x.toString());
			else			tdel(linmap, Math.floor(Math.random()*M).toString());
		}
		for(let x=0;x<N;x++) {
			const OP = Math.random();
			if(OP < WSET2)	tset(linmap, Math.floor(Math.random()*M).toString(), x.toString());
			else			tdel(linmap, Math.floor(Math.random()*M).toString());
		}
		await linmap.flush();
		await compare(linmap);
