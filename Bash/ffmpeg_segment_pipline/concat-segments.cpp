...
int main(const int argc, const char** argv) {
	...
	std::vector<Path> video_paths = getPathsSorted(getPathsFiltered(input_paths, video_substr));
	std::vector<Path> audio_paths = getPathsSorted(getPathsFiltered(input_paths, audio_substr));
	String common_name = getCommonFilename(input_paths);
	...
	for(Path path : input_paths) {
		Path dst = Path(dir_done).append(path.filename().string());
		moveFile(path, dst);
	}
